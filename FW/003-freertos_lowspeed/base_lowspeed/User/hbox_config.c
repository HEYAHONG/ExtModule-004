...
void systick_init (void)
{
    ...
    NVIC_SetPriority(Software_IRQn, 1);
...
void systick_sr_clear (void)
{
    SysTick->SR = 0;
    SysTick->CTLR |= (0xF);
}

void systick_swi_enable(void)
{
    SysTick->CTLR |= (1UL << (31));
}

void systick_swi_disable(void)
{
    SysTick->CTLR &= (~(1UL << (31)));
}

void pfic_gihwstknen(void)
{
    /*
     * 关闭硬件自动压栈
     */
     __asm volatile ("csrs 0x804, %0" : : "r" (0x1UL<<5) );
}
